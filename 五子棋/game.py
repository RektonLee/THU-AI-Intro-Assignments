...
class Board(State):
    ...
    def get_next_state(self, action) -> 'Board':
        board = deepcopy(self)
        board._states[action] = board._current_player
        board._availables.remove(action)
        board._current_player = (
            board._players[0] if board._current_player == board._players[1]
            else board._players[1]
        )
        board._last_move = action
        return board
...
class Game(object):
    ...
    def start_play(self, player1: Player, player2: Player, start_player=0, is_shown=1):
        ...
        while True:
            ...
            self.board = self.board.get_next_state(move)
            if is_shown:
                self.graphic(self.board, player1.player, player2.player)
